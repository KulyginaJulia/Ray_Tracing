...
bool Raytrace ( SRay ray, SSphere spheres[2], STriangle triangles[12], SMaterial materials[6], float start, float final, inout SIntersection intersect )
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if( (tmp) && (test < intersect.Time) )
		{
			...
			intersect.LightCoeffs = vec4(1.0, 1.0, 1.0, 512);
...
void initializeDefaultLightMaterials(out SLight light, out SMaterial materials[6])
{
	...
	materials[4].RefractionCoef = 0;
	materials[4].MaterialType = DIFFUSE_REFLECTION;
	
	materials[5].Color = vec3(0.0, 0.5, 0.5);
...
void main ( void )
{
	...
	{
		StackOfRay.data[i].ray.Origin = vec3(0.0, 0.0, 0.0);
		StackOfRay.data[i].ray.Direction = vec3(0.0, 0.0, 0.0);
		StackOfRay.data[i].contribution = 1;
		StackOfRay.data[i].depth = 0;
 	}
	StackOfRay.size = 0;
